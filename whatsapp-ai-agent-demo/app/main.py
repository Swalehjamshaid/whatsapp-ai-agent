...
logger.info("✅ Jinja2Templates initialized for dashboard")
...
try:
    ...
    if os.path.exists(dashboard_path):
        ...
        logger.warning(f"   ⚠️ dashboard.html NOT found at: {dashboard_path} - will use fallback")
    logger.info("=" * 60)
    ...
    raise
# ==========================================================
# BLOCK 51: DASHBOARD DATA ENDPOINT (Using DashboardService)
# ==========================================================

from app.services.dashboard_service import DashboardService

# Create a single instance of the service (reused for all requests)
...
async def dashboard_api_data(
    ...
    Uses DashboardService v4.2 which queries PostgreSQL directly.
    """
    print("🔔 /dashboard/api/data HIT (using DashboardService)")

    # Build filters dict, removing None values
    filters = {k: v for k, v in locals().items() if v is not None and k != "request"}
    # Remove the 'request' key if present
    filters.pop("request", None)

    try:
        data = await _dashboard_service.get_dashboard_data(filters)
        return data
    except Exception as e:
...
print("✅ Dashboard data endpoint using DashboardService")
print("=" * 60)
