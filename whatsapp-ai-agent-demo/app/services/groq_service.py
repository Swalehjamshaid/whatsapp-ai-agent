...
# VERSION: 8.2 - IMPROVED PARSING FOR PRODUCT-WISE QUERIES
# PURPOSE: Answer any logistics question using LLM + dynamic SQL.
#          All responses are generated by Groq for natural, insightful answers.
#          Enhanced to handle "product wise sales", "product sales", etc.
# ============================================================
...
Important: 
- If the user asks for "product wise sales", "sales by product", "product sales breakdown", treat it as intent='ranking', entity_type='division' (or 'model' if a specific product model is mentioned), and metric='revenue'.
- If the user asks for "product sales" without "wise" or "breakdown", it could still mean ranking by product – default to ranking by division.
...
"""
    ...
    def _parse_with_fallback(self, query: str) -> QueryIntent:
        ...
        if re.search(r"product\s*(?:wise|breakdown|by\s+product)|sales\s+by\s+product", q):
            intent = "ranking"
            entity_type = "division"  # default to division (product category)
            metric = "revenue"
            sort_by = "revenue"
            # Try to extract specific division if mentioned
            for div in ["washing machine", "refrigerator", "air conditioner", "tv", "freezer", "commercial ac", "small appliances", "drum washing machine", "water systems", "cooking"]:
                if div in q:
                    ...
                    break
            # If no specific division, leave entity_value null – will group by all divisions
            ...
            for ent in ["dealer", "model", "division", "city", "warehouse", "sales office", "sales manager"]:
                if ent in q:
                    ...
                    break
            # If no entity found, default to division
            if not entity_type:
        ...
        # If still not set, try to detect simple "product sales" as ranking by division
        ...
        # Extract metric from query if not already set
        if not metric:
...
class SQLBuilder:
    ...
    def _build_ranking(self, intent: QueryIntent) -> Tuple[str, Dict[str, Any]]:
        ...
        # If entity_value is set, add a filter for that specific value
...
class GroqResponseGenerator:
    ...
    def generate_answer(query: str, intent: QueryIntent, results: List[Dict]) -> Optional[str]:
        ...
        if intent.intent == "summary" or intent.intent == "aggregate":
            ...
            top = results[:10]  # limit to avoid token overflow
            rows = []
            for r in top:
...
class GroqService:
    ...
    def process_whatsapp_query(self, message: str, sender: str = "default") -> str:
        try:
            ...
            # 3. Generate answer using Groq (preferred)
