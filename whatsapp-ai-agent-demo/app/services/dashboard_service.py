...
class BusinessRulesConfig:
    """Approved logistics targets and weights (Enterprise Timeline Business Rules)."""

    # Time targets (in days)
    pgi_target_days: float = 1.0          # DN create → PGI
    transit_target_days: float = 2.0      # Base, overridden by distance
    pod_target_days: float = 1.0          # POD collection
    cycle_target_days: float = 4.0        # DN create → POD

    # Distance‑based transit targets (Rule 11)
...
class DashboardRepository:
    ...
    def _where(self, filters: Optional[Mapping[str, Any]], alias: str = "dr") -> Tuple[str, Dict[str, Any]]:
        ...
        # Rule 15: Exception Handling – Exclude invalid dates and negative durations
        ...
        clauses.append(
            f"({alias}.delivery_status NOT ILIKE 'Delivered' AND {alias}.pod_date IS NOT NULL "
            f"OR {alias}.delivery_status ILIKE 'Delivered' OR {alias}.pod_date IS NULL)"
        )
    ...
    def _metrics_select(alias: str = "dr") -> str:
        ...
        return f"""
            ...
            -- Average Delivery Transit Time (PGI → POD)
            COALESCE(AVG(CASE
            ...
            -- Delivery Compliance (distance‑based target)
            COALESCE(100.0 * COUNT(*) FILTER (
            ...
            -- POD Compliance (100% if POD exists after delivery)
            COALESCE(100.0 * COUNT(*) FILTER (WHERE {alias}.pod_date IS NOT NULL)
                / NULLIF(COUNT(*) FILTER (WHERE {alias}.delivery_status ILIKE 'Delivered'), 0), 0) AS pod_compliance_pct
        """
    ...
    def fetch_delivery_compliance(self, filters: Optional[Mapping[str, Any]] = None) -> List[Dict[str, Any]]:
        """Compliance by distance band (Rule 11)."""
        where, params = self._where(filters)
        quantity = self._quantity("dr")
...
class BusinessRuleEngine:
    ...
    def health_score(metrics: DashboardMetrics) -> float:
        ...
        cycle_efficiency = max(0.0, 100.0 - (metrics.avg_cycle_days * 10.0))  # Normalized
        value = (
...
# ---------------------------------------------------------------------------
# BLOCK 8: Alert Engine (Threshold‑based)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# BLOCK 10: Response Builder – Compose Final JSON for Frontend
# ---------------------------------------------------------------------------
    ...
    def build(
        ...
        return {
            ...
            "executive_summary_detailed": {
                ...
                "overall_delivery": metrics.delivery_pct,
                "overall_cycle": _round(metrics.avg_cycle_days),
            ...
            "warehouse_preview": [
                {
                    ...
                    "transit_days": row.get("avg_transit_days"),
                    "cycle_days": row.get("avg_cycle_days"),
                    "delivery_performance": row.get("status"),
...
async def health_check() -> Dict[str, str]:
    return {"status": "healthy", "version": "29.0", "timestamp": datetime.utcnow().isoformat()}
