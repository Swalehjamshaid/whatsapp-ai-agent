==========================================================
FILE: app/services/analytics_service.py (v13.0 - MASTER ANALYTICS BRAIN)
==========================================================
PURPOSE: PRIMARY ANALYTICS ENGINE - Direct PostgreSQL Integration
VERSION: 13.0 - Master Analytics Brain with High Performance
...
100% INTEGRATED WITH AI ROUTER v19.0
==========================================================
...
def init(self, dealer_name: str, error_id: str = None):
self.dealer_name = dealer_name
self.error_id = error_id or str(uuid.uuid4())[:8]
super().init(f"Dealer '{dealer_name}' not found (Error ID: {self.error_id})")

class DashboardGenerationError(AnalyticsError):
def init(self, dealer_name: str, reason: str, error_id: str = None):
self.dealer_name = dealer_name
self.reason = reason
self.error_id = error_id or str(uuid.uuid4())[:8]
super().init(f"Dashboard generation failed for '{dealer_name}': {reason} (Error ID: {self.error_id})")

class DatabaseQueryError(AnalyticsError):
def init(self, query: str, error: str, error_id: str = None):
self.query = query
self.error = error
self.error_id = error_id or str(uuid.uuid4())[:8]
super().init(f"Database query failed: {error} (Error ID: {self.error_id})")

==========================================================
RESPONSE CONTRACT
==========================================================
class AnalyticsResponse:
def init(self, success: bool = True, data: Dict[str, Any] = None, error: str = None, error_id: str = None, recovery_attempts: int = 0, groq_fallback: bool = False):
...
==========================================================
DISTANCE ENGINE (Haversine Formula + Geopy)
==========================================================
...
Calculate distance using Geopy if available
if GEOPY_AVAILABLE:
try:
distance = geodesic(
(warehouse_coords[0], warehouse_coords[1]),
(dealer_coords[0], dealer_coords[1])
).kilometers
except:
...
self._set_cached(cache_key, dealers)
return dealers
except Exception as e:
...
self._set_cached(cache_key, cities)
return cities
except Exception as e:
...
self._set_cached(cache_key, warehouses)
return warehouses
except Exception as e:
...
try:
cache_key = "control_tower_alerts"
cached = self._get_cached(cache_key)
if cached:
return cached
...
dashboard = {"trends": {"monthly": monthly}}
self._set_cached(cache_key, dashboard)
return dashboard
except Exception as e:
...
"diskcache_available": DISKCACHE_AVAILABLE and self._disk_cache is not None
}
except Exception as e:
...
def init(self, use_redis: bool = False):
...
logger.info("=" * 70)
logger.info("AnalyticsService v13.0 - Master Analytics Brain")
logger.info("=" * 70)
logger.info("")
logger.info(" ROLE: Analytics Brain - NEVER calls Groq")
logger.info("")
logger.info(" ✅ DATABASES:")
logger.info(" - PostgreSQL: Primary data store")
logger.info(" - Polars: 10x faster analytics")
logger.info(" - DuckDB: Heavy aggregations")
logger.info(" - Redis: Distributed caching")
logger.info(" - DiskCache: Fallback cache")
logger.info("")
logger.info(" ✅ ENGINES:")
logger.info(" - Distance Engine (Haversine + Geopy)")
logger.info(" - Transit Time Engine")
logger.info(" - Risk Engine")
logger.info(" - Ranking Engine")
logger.info(" - Forecasting Engine")
logger.info(" - Dealer Matching (RapidFuzz)")
logger.info("")
logger.info(" ✅ INTEGRATED WITH:")
logger.info(" - AI Router v19.0")
logger.info(" - 100% Compatible")
logger.info("")
...
return AnalyticsResponse(success=True, data=cached, recovery_attempts=0)
...
Cache the result
self.repo._set_cached(cache_key, data)
...
status = {"status": "healthy", "timestamp": datetime.now().isoformat(), "version": "13.0", "environment": "Railway" if self.is_railway else "Local", "checks": {}}
try:
db_health = self.repo.debug_database()
...
"redis_available": REDIS_AVAILABLE and self.repo._redis_client is not None,
"diskcache_available": DISKCACHE_AVAILABLE and self.repo._disk_cache is not None,
"polars_available": POLARS_AVAILABLE,
"duckdb_available": DUCKDB_AVAILABLE,
"rapidfuzz_available": RAPIDFUZZ_AVAILABLE,
"version": "13.0",
