# ==========================================================
# FILE: app/services/ai_provider_service.py (v14.0 - FULLY ALIGNED)
# ==========================================================
...
# 17. ✅ Full alignment with schema_service.py v8.0
# 18. ✅ customer_name = Dealer Name = Sold-To Party
# ==========================================================
...
class AIOrchestrator:
    ...
    # ==========================================================
    # DIRECT DEALER RESOLUTION (Bypasses SchemaService Cache)
    # ==========================================================
    
    def _resolve_dealer_direct(self, dealer_input: str, req_id: str) -> Optional[str]:
        """
        DIRECT DEALER RESOLUTION - Queries PostgreSQL via schema_service.
        This bypasses cache and goes directly to the database.
        ✅ customer_name = Dealer Name = Sold-To Party
        """
        try:
            logger.info(f"[{req_id}] 🔍 DIRECT DEALER SEARCH: '{dealer_input}'")
            
            # Use schema_service's direct database method
            resolved = self.schema.resolve_dealer_direct(dealer_input)
            
            if resolved:
                logger.info(f"[{req_id}] ✅ Direct dealer resolution: '{resolved}'")
                return resolved
            
            logger.warning(f"[{req_id}] ❌ No dealer found directly for: '{dealer_input}'")
            return None
            
        except Exception as e:
            logger.error(f"[{req_id}] Direct dealer search failed: {e}")
            return None
    ...
    def _resolve_dealer_with_retry(self, dealer_input: str, req_id: str) -> Tuple[Optional[str], float, str]:
        ...
        Attempt 1: Exact Match (SchemaService cache)
        Attempt 2: ILIKE Match (SchemaService)
        Attempt 3: Wildcard Match (SchemaService)
        Attempt 4: Normalized Match (remove special chars)
        Attempt 5: Direct Database (bypasses cache)
        """
        ...
        # Attempt 2: ILIKE Match via SchemaService
        try:
            # Use schema_service's resolve_dealer which handles ILIKE
            resolved = self.schema.resolve_dealer(dealer_clean)
            if resolved:
                confidence = 0.95
                self.metrics["dealer_resolution_success"] += 1
        ...
        # Attempt 3: Wildcard Match via Analytics
        try:
        ...
        # Attempt 5: Direct Database (bypasses all caches)
        try:
            resolved = self._resolve_dealer_direct(dealer_clean, req_id)
            if resolved:
                confidence = 0.95
                self.metrics["dealer_resolution_success"] += 1
                logger.info(f"[{req_id}] ✅ Attempt 5 (Direct DB): '{resolved}' (conf: {confidence:.2f})")
                self.dealer_resolution_cache[cache_key] = (resolved, confidence, time.time())
...
"""
    ...
    def _format_dn_details(self, data, req_id: str) -> str:
        ...
        try:
            ...
            # Extract fields - using customer_name (Sold-to Party)
