# ==========================================================
# FILE: app/services/ai_query_service.py (COMPLETE FIX)
# ==========================================================
...
# Safe import for AI provider - NO CIRCULAR IMPORT
try:
...
class IntentClassifier:
    ...
    def classify(cls, question: str, memory: Dict = None, logistics_service=None) -> Tuple[str, Optional[str]]:
        ...
        for pattern in cls.DN_PATTERNS:
            ...
            if match:
                if re.match(r'\d+$', match.group(1)):  # Is a number
                    return "DN", match.group(1)
                else:
                    # Extract dealer name from "which DN belong to X"
        ...
        executive_keywords = ["executive", "ceo", "command center", "what should i focus", "overview", "kpi"]
        ...
        pod_keywords = ["pod", "acknowledgement", "proof of delivery"]
        if any(kw in question_lower for kw in pod_keywords):
            return "POD", None
        
        # PRIORITY 7: Warehouse Query
        warehouse_keywords = ["warehouse", "godown", "stock location"]
        ...
        pending_keywords = ["pending", "backlog", "waiting"]
        ...
        # Try fuzzy matching for dealer names with typos
        if logistics_service and hasattr(logistics_service, 'get_all_dealer_names'):
            try:
                all_dealers = logistics_service.get_all_dealer_names()
                if all_dealers:
                    matches = get_close_matches(question_original, all_dealers, n=1, cutoff=0.6)
                    if matches:
                        logger.info(f"Fuzzy dealer match: '{question_original}' -> '{matches[0]}'")
                        return "DEALER", matches[0]
            except Exception as e:
                logger.debug(f"Fuzzy dealer search error: {e}")
        
        # Single word lookup
        words = question_original.strip().split()
        if len(words) == 1 and 2 < len(question_original) < 30:
            if logistics_service and hasattr(logistics_service, 'search_dealer'):
                try:
                    dealer_match = logistics_service.search_dealer(question_original)
                    if dealer_match:
        ...
        if memory and memory.get("last_intent"):
            follow_up_keywords = ["why", "how", "what about", "tell me more", "explain"]
...
class ResponseFormatter:
    ...
    def unknown_response() -> str:
        return "❓ I couldn't identify your request.\n\nType `services` to see what I can do."
    ...
    def warehouse_response(warehouse_data: Dict) -> str:
        return f"🏭 *WAREHOUSE: {warehouse_data.get('warehouse')}*\n\n📊 Total DNs: {warehouse_data.get('total_dns', 0)}\n⏳ Pending: {warehouse_data.get('pending_dns', 0)}\n⚡ Efficiency: {warehouse_data.get('efficiency_score', 0)}%"
...
class AIQueryService:
    ...
    def __init__(self, db: Session):
        ...
        # CRITICAL FIX: Set ai_available FIRST
    ...
    def _check_deepseek_health(self):
        ...
        try:
            ...
            if response.get("success"):
                ...
                logger.error(f"❌ DeepSeek health check FAILED")
        except Exception as e:
            self.ai_available = False
    ...
    def process_query(self, question: str, user_phone: str = None) -> Dict[str, Any]:
        ...
        # FIXED: Database logging with correct field names
    ...
    def _handle_warehouse_query(self, warehouse_name: str, user_phone: str = None) -> Dict[str, Any]:
        try:
            if hasattr(self.analytics, 'warehouse_rankings'):
                ...
                return {"success": False, "response": f"❌ Warehouse analytics not available.", "ai_used": False}
        except Exception as e:
            logger.error(f"Warehouse error: {e}")
            return {"success": False, "response": f"❌ Unable to fetch warehouse data.", "ai_used": False}
    ...
    def _handle_product_query(self, product_name: str, user_phone: str = None) -> Dict[str, Any]:
        try:
            if hasattr(self.analytics, 'product_dashboard'):
                ...
                return {"success": False, "response": f"❌ Product analytics not available.", "ai_used": False}
        except Exception as e:
            logger.error(f"Product error: {e}")
            return {"success": False, "response": f"❌ Unable to fetch product data.", "ai_used": False}
    ...
    def _log_query(self, question: str, result: Dict, user_phone: str = None, conversation_id: int = None):
        """Log query to database - FIXED to use correct field names"""
        try:
            log_entry = AIResponseLog(
                conversation_id=conversation_id,  # Can be None
                prompt=question[:500],            # FIXED: Use 'prompt' not 'question'
                ai_response=result.get("response", "")[:2000],  # FIXED: Use 'ai_response' not 'response'
