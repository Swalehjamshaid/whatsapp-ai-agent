...
class MasterRouter:
    ...
    def _register_routes(self):
        ...
        self._routes[QueryType.DN_RECOMMENDATIONS] = self.service._handle_dn_recommendations
        ...
        self._routes[QueryType.PENDING_PGI_BY_DAYS] = self.service._handle_pending_pgi_by_days
        ...
        self._routes[QueryType.DEALER_RANKING] = self.service._handle_dealer_ranking
        self._routes[QueryType.DEALER_RISK] = self.service._handle_dealer_risk
        
        # Warehouse routes
        self._routes[QueryType.WAREHOUSE_RANKING] = self.service._handle_warehouse_ranking
        self._routes[QueryType.TOP_WAREHOUSES] = self.service._handle_top_warehouses
        self._routes[QueryType.WAREHOUSE_DELAY] = self.service._handle_warehouse_delay
        
        # Product routes
        self._routes[QueryType.TOP_PRODUCTS] = self.service._handle_top_products
        self._routes[QueryType.PRODUCT_RANKING] = self.service._handle_product_ranking
        ...
        self._routes[QueryType.KPI_DASHBOARD] = self.service._handle_kpi_dashboard
        self._routes[QueryType.REVENUE_ANALYSIS] = self.service._handle_revenue_analysis
        
        # Control tower
        self._routes[QueryType.CONTROL_TOWER] = self.service._handle_control_tower
        self._routes[QueryType.ALERTS] = self.service._handle_alerts
        
        # AI Analysis
        self._routes[QueryType.ROOT_CAUSE_ANALYSIS] = self.service._handle_root_cause_analysis
        self._routes[QueryType.TREND_ANALYSIS] = self.service._handle_trend_analysis
        self._routes[QueryType.RECOMMENDATIONS] = self.service._handle_recommendations
        self._routes[QueryType.PREDICTIVE_ANALYSIS] = self.service._handle_predictive_analysis
...
class AIQueryService:
    ...
    def _handle_dn_recommendations(self, entities: ExtractedEntities, question: str) -> Dict:
        if not entities.dn_number:
            return self.formatter.error("Please provide a DN number")
        ...
        if self.logistics_service:
            ...
            return self.formatter.success(
                ...
                summary="\n".join([f"• {r}" for r in recs]) if recs else "No recommendations needed"
            )
    ...
    def _handle_pending_pgi_by_days(self, entities: ExtractedEntities, question: str) -> Dict:
        if not entities.days:
            return self.formatter.error("Please specify number of days")
        ...
        if self.logistics_service:
            ...
            return self.formatter.success(
                data={"pending_pgi": filtered, "days_threshold": entities.days, "count": len(filtered)},
                summary=f"Found {len(filtered)} PGI pending >{entities.days} days"
            )
    ...
    def _handle_dealer_ranking(self, entities: ExtractedEntities, question: str) -> Dict:
        return self._handle_top_dealers(entities, question)
    
    def _handle_dealer_risk(self, entities: ExtractedEntities, question: str) -> Dict:
        if not entities.dealer:
            return self.formatter.error("Please specify a dealer name")
        ...
        if self.analytics_service:
            result = self.analytics_service.get_dealer_dashboard(entities.dealer)
            if result:
                total_dns = result.get("total_dns", 0)
                completed = result.get("completed_dns", 0)
                ...
                return self.formatter.success(
                    data={"dealer": entities.dealer, "pending_dns": pending, "risk": risk},
                    summary=f"Dealer {entities.dealer}: {risk} risk ({pending} pending DNs) {icon}"
                )
    ...
    def _handle_top_warehouses(self, entities: ExtractedEntities, question: str) -> Dict:
        return self._handle_warehouse_ranking(entities, question)
    
    def _handle_warehouse_delay(self, entities: ExtractedEntities, question: str) -> Dict:
        if not entities.warehouse:
            return self.formatter.error("Please specify a warehouse")
        
        return self.formatter.success(
            data={"warehouse": entities.warehouse},
            summary=f"🏭 *Warehouse: {entities.warehouse}*\n\nDelay analysis in progress."
        )
    
    def _handle_top_products(self, entities: ExtractedEntities, question: str) -> Dict:
        if self.analytics_service:
    ...
    def _handle_product_ranking(self, entities: ExtractedEntities, question: str) -> Dict:
        return self._handle_top_products(entities, question)
    ...
    def _handle_kpi_dashboard(self, entities: ExtractedEntities, question: str) -> Dict:
        return self._handle_executive_dashboard(entities, question)
    
    def _handle_revenue_analysis(self, entities: ExtractedEntities, question: str) -> Dict:
        if self.analytics_service:
    ...
    def _handle_alerts(self, entities: ExtractedEntities, question: str) -> Dict:
        return self._handle_control_tower(entities, question)
    
    def _handle_root_cause_analysis(self, entities: ExtractedEntities, question: str) -> Dict:
        ...
        if self.logistics_service:
            ...
            is_valid, _, pod_items = self.validator.validate_list_data(pod_result, "pending_pods")
            if is_valid:
                data["pending_pods"] = pod_items[:10]
            
            pgi_result = self.logistics_service.get_pending_pgi()
            is_valid, _, pgi_items = self.validator.validate_list_data(pgi_result, "pending_pgi")
            if is_valid:
                data["pending_pgi"] = pgi_items[:10]
        
        return self.formatter.success(
            data=data,
            summary="Root cause analysis will be provided by AI."
        )
    
    def _handle_trend_analysis(self, entities: ExtractedEntities, question: str) -> Dict:
        return self.formatter.success(
            data={},
            summary="Trend analysis will be provided by AI."
        )
    
    def _handle_recommendations(self, entities: ExtractedEntities, question: str) -> Dict:
        return self.formatter.success(
            data={},
            summary="Recommendations will be provided by AI."
        )
    
    def _handle_predictive_analysis(self, entities: ExtractedEntities, question: str) -> Dict:
        return self.formatter.success(
            data={},
            summary="Predictive analysis will be provided by AI."
        )
    ...
    def _handle_universal(self, entities: ExtractedEntities, question: str) -> Dict:
        ...
        if entities.city:
            return self._handle_city_dashboard(entities, question)
        if entities.warehouse:
            return self._handle_warehouse_delay(entities, question)
...
• `Pending POD > 7 days` - Old pending PODs
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
*Powered by Enterprise Logistics Intelligence v31.0*
"""
