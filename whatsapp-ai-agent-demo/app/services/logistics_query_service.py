# ==========================================================
# FILE: app/services/logistics_query_service.py (v1.1 - DATABASE ACCESS LAYER)
# ==========================================================
...
class LogisticsQueryService:
    ...
    def get_dealer_dns(self, dealer_name: str, limit: int = 20) -> List[Dict]:
        try:
            ...
            ).filter(DeliveryReport.customer_name == dealer_name).order_by(
                desc(DeliveryReport.dn_create_date)
            ).limit(limit).all()
            return [{
                "dn_no": r.dn_no, "dn_date": r.dn_create_date,
                "pgi_date": r.good_issue_date, "pod_date": r.pod_date,
                "units": int(r.dn_qty or 0), "amount": float(r.dn_amount or 0),
                "warehouse": r.warehouse, "city": r.ship_to_city
            } for r in results]
        except Exception as e:
    ...
    def get_dn_details(self, dn_number: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            return {
                "dn_number": record.dn_no, "dealer": record.customer_name,
                "warehouse": record.warehouse, "city": record.ship_to_city,
                "units": int(record.dn_qty or 0), "amount": float(record.dn_amount or 0),
                "dn_date": record.dn_create_date, "pgi_date": record.good_issue_date,
                "pod_date": record.pod_date, "delivery_aging": delivery_aging,
                "pod_aging": pod_aging, "status": status,
    ...
    def get_critical_deliveries(self, threshold_days: int = 15, limit: int = 10) -> List[Dict]:
        try:
            results = self.db.query(
                DeliveryReport.dn_no, DeliveryReport.customer_name,
                DeliveryReport.warehouse,
                func.datediff(self.today, DeliveryReport.dn_create_date).label('aging')
            ).filter(
                DeliveryReport.good_issue_date.is_(None),
                DeliveryReport.dn_create_date.isnot(None),
                func.datediff(self.today, DeliveryReport.dn_create_date) > threshold_days
